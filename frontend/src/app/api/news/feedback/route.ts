...
export async function POST(req:NextRequest){
 ...
 const r=await fetch(`${process.env.BACKEND_URL||'http://127.0.0.1:8001'}/news/feedback`,{method:'POST',headers:{'Content-Type':'application/json','X-Chat-Token':key,'X-Client-Key':client},body,signal:AbortSignal.timeout(60000),cache:'no-store'});
